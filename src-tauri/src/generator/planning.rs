...
async fn generate_layout_plans(
    ...
    let aspect_ratio_hint = match config.aspect_ratio {
        ...
        crate::types::AspectRatio::Ratio32x9 => {
            ...
            PREFERRED layouts for 32:9 (they naturally use horizontal space well):\n\
            - step_flow, timeline, process: steps/events spread naturally across the wide canvas — strongly prefer these for sequential/workflow content\n\
            - feature_grid, section_intro, outcome_grid: can hold 6-8 cards/columns instead of 4\n\
            - center_grid: can hold 6 items instead of 4\n\
            ACCEPTABLE with care:\n\
            - split_layers: left panel narrower (~25%), right layer diagram wider (~75%)\n\
            - compare, focus_example: two-panel layouts still work but may feel sparse — only use when content truly demands them\n\
            AVOID:\n\
            - spotlight: single image + narrow panels looks awkward at this width\n\
            Layout column counts: feature_grid 6-8, center_grid 6, outcome_grid 4-6, section_intro 6-8. All horizontal padding increased by 50%."
        }
        crate::types::AspectRatio::Ratio48x9 => {
            "\n\nULTRA-WIDE FORMAT GUIDANCE (48:9 aspect ratio):\n\
            PREFERRED layouts for 48:9 (fill the panoramic width naturally):\n\
            - step_flow, timeline, process: ideal — steps/events spread elegantly across the full panoramic width\n\
            - feature_grid, section_intro, outcome_grid: can hold 8-12 cards/columns\n\
            - center_grid: can hold 8 items\n\
            AVOID — these fixed left-right layouts leave extreme whitespace at 48:9:\n\
            - focus_example: left-points + right-example two-panel layout becomes too sparse and stretched\n\
            - compare: two equal panels at 48:9 result in very wide, sparsely populated columns\n\
            - spotlight: single image + narrow panels is inappropriate at panoramic width\n\
            - split_layers: if used, left panel must be very narrow (~20%), right extremely wide (~80%)\n\
            Layout column counts: feature_grid 8-12, center_grid 8, outcome_grid 6-8, section_intro 8-12. All horizontal padding doubled."
        }
    };
...
pub async fn layout_one_page(
    ...
    let aspect_constraint = match config.aspect_ratio {
        crate::types::AspectRatio::Ratio16x9 => "",
        crate::types::AspectRatio::Ratio32x9 => {
            "\n\nWIDE FORMAT (32:9): Prefer step_flow/timeline/process/feature_grid/outcome_grid. \
             Avoid spotlight. Use focus_example/compare only when content truly demands two-panel layout."
        }
        crate::types::AspectRatio::Ratio48x9 => {
            "\n\nULTRA-WIDE FORMAT (48:9): Strongly prefer step_flow/timeline/process/feature_grid/outcome_grid/section_intro. \
             AVOID focus_example, compare, spotlight — these fixed two-panel or single-image layouts create extreme whitespace at panoramic width."
        }
    };
